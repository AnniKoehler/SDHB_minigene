...
def gtf_for_ggplot(annotation, start, end, arrow_bins):
        ...
        gtfp = gtfp + theme(
                ...
                axis.text.y     = element_blank(),   # <– IDs verschwinden
                axis.title.y    = element_blank()    # <– Y-Titel weg
        )
        """ %(start, end)
...
#       args.bam = "/nfs/no_backup/rg/epalumbo/projects/tg/work/8b/8b0ac8705f37fd772a06ab7db89f6b/2A_m4_n10_toGenome.bam"
        ...
        for id, bam, overlay_level, color_level, label_text in read_bam_input(args.bam, args.overlay, args.color_factor, args.labels):
                ...
                # Wenn ein SJ-File über -j/--junctions-bed übergeben wurde,
                if args.junctions_bed:
                        # Lade Junction-Counts aus SJ.out.tab und mappe STAR-Strandcodes auf '+'/'-'
                        file_junc = {'+': {}, '-': {}}
                        with open(args.junctions_bed) as f:
                                for line in f:
                                        fields = line.strip().split()
                                        start = int(fields[1])
                                        end = int(fields[2])
                                        # STAR SJ.out.tab col4: 0 unknown, 1 forward, 2 reverse
                                        code = fields[3]
                                        strand = '+' if code == '1' else '-' if code == '2' else '+'
                                        cnt = int(fields[6])  # unique-mapping read count
                                        file_junc[strand][(start, end)] = cnt
                        # Überschreibe die BAM-basierten Junctions
                        junctions = file_junc
        ...
        for strand in bam_dict:
                ...
                R_script += """
                               ...
                               for (bam_index in 1:length(density_list)) {
                                       ...
                                       gp = gp + labs(y=NULL, title = labels[[id]]) +
                                                theme(plot.title = element_text(hjust=0, vjust=0.5, margin=margin(0,0,4,0)))
                                       if(exists('coord_dict')) {
                               ...
                               #               gp = gp + annotation_custom(grob = rectGrob(x=0, y=0, gp=gpar(col="green"), just=c("left","bottom")), j[1], xmid, j[3], ymid)
                                   ...
                                   # Coverage-Panels sind schon in density_grobs

                               # Coverage-Panels bleiben in density_grobs[1:length(density_list)]

                            # Dann die X-Achse
                            density_grobs[["xaxis"]] <- xaxisGrob
                        
                            # Spacer-Grob (leerer Zwischenraum)
                            spacer <- nullGrob()  # leeres Grob für Abstand
                        
                            # Ganz unten die Annotation (falls vorhanden)
                            if (%(args.gtf)s == 1) {
                                gtfGrob <- ggplotGrob(gtfp)
                                maxWidth <- grid::unit.pmax(
                                    maxWidth,
                                    gtfGrob$widths[2+vs] + gtfGrob$widths[3+vs]
                                )
                                density_grobs[["gtf"]] <- gtfGrob
                            }
                        
                            # --- HARTE ANGLEICHUNG DER BREITEN: referenziere 1. Coverage-Panel ---
                                # Referenz-Breiten vom ersten Coverage-Panel holen
                                ref_id <- names(density_grobs)[1]
                                ref_widths <- density_grobs[[ref_id]]$widths
                                
                                # X-Achse als eigenes Grob aufnehmen (direkt nach den Coverage-Panels)
                                density_grobs[["xaxis"]] <- xaxisGrob
                                
                                # GTF-Grob (falls vorhanden) erzeugen und aufnehmen
                                if (%(args.gtf)s == 1) {
                                  gtfGrob <- ggplotGrob(gtfp)
                                  density_grobs[['gtf']] <- gtfGrob
                                }
                                
                                # Harte Angleichung: wo möglich komplette widths an Referenz koppeln,
                                # sonst mindestens Panel-Spalte (2+vs) und Y-Text-Spalte (3+vs) angleichen
                                for (id in names(density_grobs)) {
                                  w <- density_grobs[[id]]$widths
                                  if (length(w) == length(ref_widths)) {
                                    density_grobs[[id]]$widths <- ref_widths
                                  } else {
                                    # Fallback: gleiche kritische Spalten an
                                    density_grobs[[id]]$widths[2+vs] <- ref_widths[2+vs]
                                    density_grobs[[id]]$widths[3+vs] <- ref_widths[3+vs]
                                  }
                                }
                                # --- Ende harte Angleichung ---
                                
                                # Höhen definieren (Coverage -> X-Achse -> GTF [optional])
                                heights <- unit.c(
                                ...
                                # Reihenfolge: alle Coverage-Panels in ihrer bestehenden Reihenfolge,
                                # dann die X-Achse, dann optional GTF
                                grob_order <- c(
                                  names(density_grobs)[1:length(density_list)],
                                  "xaxis",
                                  if (%(args.gtf)s == 1) "gtf" else NULL
                                )
                                
                                argrobs <- arrangeGrob(
                                  grobs   = density_grobs[grob_order],
                                  ncol    = 1,
                                  heights = heights
                                )
                               ...
                               # Save plot to file in the requested format
                                ...
                                ggsave("%(out)s", plot = argrobs, device = "tiff",
                                width = width, height = height, units = "in",
                                dpi = %(out_resolution)s, compression = "lzw", limitsize = FALSE)
                                } else {
                                ggsave("%(out)s", plot = argrobs, device = "%(out_format)s",
                                width = width, height = height, units = "in",
                                dpi = %(out_resolution)s, limitsize = FALSE)
                                }
                                dev.off()
